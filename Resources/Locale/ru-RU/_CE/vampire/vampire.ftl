ce-heat-under-sun = Солнечный свет нестерпимо жжётся...

ce-vampire-examine = [color=red]Ярко красные глаза и длинные клыки говорят вам, что перед вами опаснейший вампир. Ваши инстинкты кричат вам бежать или сражаться![/color]

ce-magic-spell-need-vampire-valid = Недоступно в скрытой форме вампира.
ce-magic-spell-need-ignited-vamp-candles = Необходимо {$count} зажженных вампирских свечей вокруг.

ce-vampire-tree-examine-friend = [color=green]Это сердце вашего клана. [/color] Защищайте его всеми силами.
ce-vampire-tree-examine-enemy = [color=red]Это сердце вражеского клана.[/color]

ce-vampire-tree-examine-level = Эссенции: ([color=red]{$essence}/{$left}[/color]). Текущий уровень клана: [color=red]{$level}[/color].

ce-vampire-essence-holder-examine = В этой жертве есть ещё [color=red]{$essence} эссенции крови[/color].

ce-vampire-tree-other-title = Прогресс других кланов:
ce-vampire-tree-other-info = - [color=red]({$essence}/{$left})[/color] эссенции, [color=red]{$lvl}[/color] уровень.
ce-vampire-gather-essence-no-left = Здесь больше нет эссенции!

## Announcements

ce-vampire-sender = Вампирское чутьё

ce-vampire-tree-created = Создано сердце клана!
ce-vampire-tree-growing = Чужое сердце клана вырастает до {$level} уровня!
ce-vampire-tree-growing-self = Ваше сердце клана вырастает до {$level} уровня!
ce-vampire-tree-damaged = Ваше сердце клана атаковано!
ce-vampire-tree-destroyed = Чужое сердце клана разрушено!
ce-vampire-tree-destroyed-self = Ваше сердце клана разрушено!

ce-admin-verb-text-make-vampire = Превратить в высшего вампира.
ce-roles-antag-vampire-briefing = Эд, не забудь вставить адекватный брифинг для вампиров. Игрок. Если ты это видишь, пингуй Эда с смешными эмодзями. и скрином этого сообщения.

ce-skill-req-higher-vampire = Только высший вампир может сделать вас сильнее.
ce-verb-categories-vampire-learn = Обучить младшего вампира
