ce-energy-overcharge-popup = Слишком много энергии!
ce-energy-deficit-popup = Недостаточно маны!

ce-magic-manacost = Затраты маны
ce-magic-cooldown = Перезарядка

ce-magic-spell-not-enough-mana = Недостаточно маны!
ce-magic-spell-no-mana-component = Вы не используете магическую энергию.

ce-magic-spell-not-enough-mana-cast-warning-0 = Всё внутри вас начинает неприятно ныть...
ce-magic-spell-not-enough-mana-cast-warning-1 = Ваша голова начинает гудеть...
ce-magic-spell-not-enough-mana-cast-warning-2 = Ваши руки дрожат...
ce-magic-spell-not-enough-mana-cast-warning-3 = К горлу подкатывает ком...
ce-magic-spell-not-enough-mana-cast-warning-4 = Голова становится тяжёлой...

ce-magic-verbal-aspect = Требуется способность говорить
ce-magic-somatic-aspect = Требуется свободные руки:
ce-magic-material-aspect = Требуются следующие материальные компоненты:
ce-magic-music-aspect = Вы должны играть на музыкальном инструменте

ce-magic-spell-need-verbal-component = Вы не можете произнести заклинание вслух.
ce-magic-spell-need-somatic-component = Ваши руки заняты.
ce-magic-spell-need-material-component = Нужно держать материальный компонент заклинания в руках.

ce-magic-spell-stamina-not-enough = У вас недостаточно сил для этого.
ce-magic-staminacost = Затраты выносливости
ce-magic-weapon-required = Необходимо держать в руках оружие
ce-magic-spell-pacified = Это может причинить кому-то вред!
ce-magic-spell-ssd = Не трогайте отключившихся игроков!

ce-magic-spell-target-not-mob = Цель должна быть живым существом!

ce-magic-spell-target-mob-state = Можно использовать только на {$state} целях!
ce-magic-spell-target-mob-state-dead = мёртвых
ce-magic-spell-target-mob-state-live = живых
ce-magic-spell-target-mob-state-critical = умирающих

ce-magic-skillpointcost = Стоимость ресурса "{$name}": [color=#eba834]{$count}[/color]
ce-magic-spell-skillpoint-not-enough = Не хватает {$count} ресурса(ов) "{$name}"!

ce-magic-essencecost = Затраты эссенции из фокусировки
ce-magic-spell-not-enough-essence = Недостаточно магической эссенции в фокусировке!
