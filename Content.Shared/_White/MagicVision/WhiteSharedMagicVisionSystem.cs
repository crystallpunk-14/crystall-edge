...
namespace Content.Shared._White.MagicVision;

public abstract class WhiteSharedMagicVisionSystem : EntitySystem
{
    ...
    public readonly EntProtoId MagicTraceProto = "WhiteMagicVisionMarker";

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<WhiteMagicVisionMarkerComponent, ExaminedEvent>(OnExamined);
        SubscribeLocalEvent<WhiteMagicVisionStatusEffectComponent, StatusEffectRelayedEvent<MobStateChangedEvent>>(OnMobStateChange);
    }

    private void OnMobStateChange(Entity<WhiteMagicVisionStatusEffectComponent> ent, ref StatusEffectRelayedEvent<MobStateChangedEvent> args)
    {
    ...
    protected virtual void OnExamined(Entity<WhiteMagicVisionMarkerComponent> ent, ref ExaminedEvent args)
    {
        ...
        sb.Append($"{Loc.GetString("white-magic-vision-timed-past")} {timePassed.Minutes}:{timePassed.Seconds:D2}\n");
        ...
        {
            sb.Append($"{Loc.GetString("white-magic-vision-aura")} {imprint}");
            args.AddMarkup(sb.ToString());
            return;
        }
        ...
        sb.Append($"{Loc.GetString("white-magic-vision-aura")} {obscuredImprint}");

        args.AddMarkup(sb.ToString());
    }

    public string? GetAuraImprint(Entity<WhiteAuraImprintComponent?> ent)
    {
        if (!Resolve(ent, ref ent.Comp))
            return null;

        if (_statusEffects.TryEffectsWithComp<WhiteHideMagicAuraStatusEffectComponent>(ent, out var hideComps))
        {
        ...
        EntityCoordinates? target = null)
    {
        ...
        var markerComp = EnsureComp<WhiteMagicVisionMarkerComponent>(ent);
