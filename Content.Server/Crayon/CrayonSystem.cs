...
public sealed partial class CrayonSystem : SharedCrayonSystem
{
    ...
    private void OnMapInit(Entity<CrayonComponent> ent, ref MapInitEvent args)
    {
        ...
        var decal = ProtoMan.EnumeratePrototypes<DecalPrototype>().FirstOrDefault(x => x.Tags.Contains("CEcrayon")); //CrystallEdge
    ...
    private void OnCrayonBoundUI(EntityUid uid, CrayonComponent component, CrayonSelectMessage args)
    {
        ...
        if (!ProtoMan.TryIndex<DecalPrototype>(args.State, out var prototype) || !prototype.Tags.Contains("CEcrayon")) //CrystallEdge
            return;
