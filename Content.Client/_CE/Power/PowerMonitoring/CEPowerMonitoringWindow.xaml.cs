...
public sealed partial class CEPowerMonitoringWindow : FancyWindow
{
    ...
    private static readonly SpriteSpecifier.Texture LeakBlip = new(new ResPath("/Textures/Interface/NavMap/beveled_triangle.png"));
    ...
    public CEPowerMonitoringWindow()
    {
        ...
        MasterTabContainer.SetTabTitle(2, Loc.GetString("ce-power-monitoring-window-tab-regulators"));
        MasterTabContainer.SetTabTitle(3, Loc.GetString("ce-power-monitoring-window-tab-distributors"));
        ...
        PowerMonitoringConsoleEntry[] focusLoads)
    {
        ...
        TotalSources.Text = Loc.GetString("ce-power-monitoring-window-value", ("value", Math.Round(totalSources).ToString("N0")));
        TotalBatteryUsage.Text = Loc.GetString("ce-power-monitoring-window-value", ("value", Math.Round(totalBatteryUsage).ToString("N0")));
        TotalLoads.Text = Loc.GetString("ce-power-monitoring-window-value", ("value", Math.Round(totalLoads).ToString("N0")));
        ...
        // Energy leaks (CE) - the map-side of the alert.
        if (NavMap.Visible)
        {
            foreach (var (netEntity, netCoords) in console.EnergyLeaks)
            {
                var coords = _entManager.GetCoordinates(netCoords);
                NavMap.TrackedEntities[netEntity] = new NavMapBlip(coords, _spriteSystem.Frame0(LeakBlip), new Color(255, 48, 48), true, false);
            }
        }
    ...
    private PowerMonitoringConsoleEntry[] GetUpdatedPowerMonitoringConsoleEntries(PowerMonitoringConsoleEntry[] entries, CEPowerMonitoringConsoleComponent console)
    {
